...
- Directory cleanup must archive both the worktree and its administrative directory: initialized submodules keep their object databases under the latter, not in their visible `.git` files. A read-only inventory loop initially had a shell syntax error and made no changes; the corrected inventory found source/evidence that needed preservation before removal.
- Parallel worktree removal can change the link count/ctime of Git pack files being archived elsewhere: matching inode `13266015` was shared between a candidate and the canonical object database. Archive creation stopped safely on that warning; retain the source and verify/retry sequentially rather than ignoring it. Also drain registry-check pipelines instead of using `grep -q` under `pipefail`, which can mistake producer SIGPIPE for an unregistered path.
...
**Filtered simulator output masked an RTL fatal (2026-09-05):** the testing recipe piped the recursive simulator through `grep`, and the lower Verilator recipe independently piped its build through `tee`, so either layer could return success after a deterministic compile or RTL fatal and launch a stale executable. Both Makefiles now use Bash with `pipefail`; require the unmasked recursive status plus the test-specific guest marker, and clean after any failed build before accepting a run.
...
**A stale nested-repository branch made a farm launch reject the intended revision (2026-09-05):** the first BlackParrot push updated a similarly named branch from another backing repository, so the synthesis source guard could not fetch the intended `06641129` commit and stopped before Vivado. Push and verify the exact commit from the candidate worktree's own nested repository before admission; the rejected launch consumed no synthesis slot.

**A nominally nonresident test used an all-resident topology (2026-09-05):** `mt_umode_nonresident_handoff_test` originally inherited the 4-thread/4-context defaults, so switching to context 2 never exercised SRAM eviction or restore and hid the FPGA failure. Run it with `NUM_THREADS=2 NUM_CONTEXTS=4`; the harness now rejects configurations where logical contexts do not exceed resident slots.

**Parallel Make raced cleanup against a test build (2026-09-05):** invoking `make -j clean run-...` allowed `clean` to remove a just-built test image while the recursive simulator flow was copying it. Run cleanup to completion in one Make invocation, then launch the parallel rebuild/run in a second invocation; never submit `clean` and a consuming target as concurrent goals.

**A long interactive console transfer exceeded the guest shell line limit (2026-09-05):** sending a complete base64-encoded ELF as one command left the minimal PID-1 shell at a continuation prompt, so no test instruction executed. Future interactive deployment must use bounded chunks with a hash check, or embed the byte-verified executable in the initramfs acceptance image.

**FST conversion accidentally streamed a multi-gigabyte VCD to a log (2026-09-05):** `fst2vcd` treats a second positional pathname as unused and writes VCD to stdout unless `-o` is supplied. Use `fst2vcd -f input.fst -o output.vcd`, check free space first, and terminate any retained reader before assuming an unlinked temporary file has released its blocks.

**Archived FST hierarchy did not match its trace (2026-09-05):** a copied `.fst.hier` came from a later run, while the FST itself contained a valid embedded hierarchy; pairing them produced misleading signal identities. Prefer the embedded hierarchy after a normal close, hash any required sidecar with the FST, and reject an archive whose hierarchy/source pair was not captured atomically.

**Packed-struct FST aliases were mistaken for scalar fields (2026-09-05):** both `fstminer` and the local direct-handle prototype can expose a packed aggregate through a field-named alias, so the returned value width exceeded the declared leaf width; a buffer sized from that leaf width corrupted memory. Validate handle widths against returned data before interpreting aliases, allocate defensively, and do not use the experimental `--at` path as evidence until this is fixed.

**PYNQ readiness check omitted the SSH user (2026-09-05):** calling the readiness helper with only the board address tried the wrong login and timed out even though the board was healthy. Always pass the complete reviewed target `xilinx@192.168.4.35`; a readiness-probe authentication failure is not a reason to power-cycle.

**First APC-fix simulation was launched without a native bound (2026-09-05):** after the translated failure changed into a nonterminal state, the run had to be interrupted and its trace was not accepted as closed evidence. Potentially stalling handoff diagnostics now start with `BSG_TRACE_TIMEOUT_S`, use text-only bounded probes for localization, and reserve a clean `TRACE=1` run for the final proof.

**Wrapper test omitted its included C dependency (2026-09-05):** editing the shared U-mode handoff body did not make the Sv39 wrapper ELF stale according to Make, so the first rerun would have reused the diagnostic-store image; it was stopped before loading completed and was not interpreted. The wrapper target now explicitly depends on the included source, and test-specific output plus ELF hashes remain required after source changes.

**Completed nonresident handoff inherited stale replay state (2026-09-06):** the first corrected translated trace reached the target redirect but an immediate ITLB miss used the victim physical CSR bank's old APC and returned to source code. Restore APC from the virtual context's saved NPC with the CSR image; validate with a cold translated target so a warm physical fetch cannot hide stale replay bookkeeping.

**Younger switch survived an older target trap (2026-09-06):** a target-side ECALL or DTLB replay redirected while the following speculative `csrwi 0x800,0` had already rearmed the nonresident FSM, causing the director to emit resume instead of trap. Cancel `e_context_cache_wait_ctxtsw_commit` on an older non-switch redirect before state installation, and limit explicit FE cancel/resume to a real launched resident token.

**Linux resume target packaged a stale PID-1 executable (2026-09-06):** image validation found that the prepared initramfs contained the older no-target-syscall ELF even though the current source and output ELF enabled the target-context syscall. `tiny-init-linux-image-resume` now installs the current tiny ELF into the prepared sysroot before rebuilding the rootfs, kernel, OpenSBI payload, and NBF; require the existing byte-comparison validation before every acceptance run.
