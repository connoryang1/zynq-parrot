module prefetch_mshr_table_tb
  (input clk_i, reset_i, alloc_v_i, issue_v_i, response_v_i, response_last_i,
   ...
   output demand_release_o, output [1:0] demand_release_id_o);
  logic [1:0] valid, issued;
  logic [1:0][55:0] addr;
  logic [1:0][1:0] ctx;
  logic [1:0][2:0] way;
  logic [1:0][3:0] fill_mask;
  bp_prefetch_mshr_table #(.addr_width_p(56), .id_width_p(2), .context_width_p(2),
    ...
    (.clk_i, .reset_i, .alloc_ready_o, .alloc_v_i, .alloc_addr_i,
     ...
     .demand_wait_o, .demand_release_o, .demand_release_id_o);
  ...
  assign fill_mask0_o = fill_mask[0]; assign fill_mask1_o = fill_mask[1];
endmodule
