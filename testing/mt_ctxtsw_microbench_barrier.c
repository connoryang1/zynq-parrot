/**
 * mt_ctxtsw_microbench_barrier.c
 *
 * Same shape as mt_ctxtsw_microbench, but treats ctxtsw CSR writes as compiler
 * memory barriers. This isolates whether the original no-output behavior is
 * caused by the optimizer moving or reusing state across ctxtsw.
 */
...
int main(void) {
  ...
  restore_gp();
  bp_print_string("=== Context Switch Barrier Microbenchmark ===\n");
