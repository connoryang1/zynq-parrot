 ...
 * AXI bursts. Ordinary reads and writes remain on the existing AXI-Lite bridge;
 * this gives detached UCE hints enough transport capacity to overlap external
 * memory latency without changing demand traffic.
 */
...
module bp_prefetch_axi_master
 ...
 #(parameter bp_params_e bp_params_p = e_bp_default_cfg
  ...
  logic [outstanding_p-1:0] slot_half_v_r;
  logic [outstanding_p-1:0][axi_data_width_p-1:0] slot_half_data_r;
  ...
  logic response_slot_valid, response_half_v;
  bp_bedrock_mem_rev_header_s response_header;
  logic [axi_data_width_p-1:0] response_half_data;
  always_comb begin
    response_slot_valid = 1'b0;
    response_half_v = 1'b0;
    response_header = '0;
    response_half_data = '0;
    if (response_id_valid) begin
      response_slot_valid = slot_v_r[response_slot];
      response_half_v = slot_half_v_r[response_slot];
      response_header = slot_header_r[response_slot];
      response_half_data = slot_half_data_r[response_slot];
    end
  end

  assign mem_rev_header_o = response_header;
  assign mem_rev_data_o = {axi_rdata_i, response_half_data};
  assign mem_rev_v_o = axi_rvalid_i & response_slot_valid & response_half_v;
  assign axi_rready_o = response_slot_valid
    & (!response_half_v | mem_rev_ready_and_i);
  ...
  always_ff @(posedge clk_i) begin
    if (reset_i) begin
      ...
      if (response_accept) begin
        ...
        if (!response_half_v) begin
          slot_half_v_r[response_slot] <= 1'b1;
          slot_half_data_r[response_slot] <= axi_rdata_i;
        end else begin
          slot_half_v_r[response_slot] <= 1'b0;
  ...
  initial begin
    ...
    if (axi_data_width_p != 32 || bedrock_fill_width_p != 64
        || dcache_block_width_p != 512 || block_beats_lp != 16)
      $fatal(1, "Prefetch AXI bridge requires 512-bit lines, 64-bit BedRock fills, and 32-bit AXI");
  end
  ...
  always_ff @(negedge clk_i) if (!reset_i) begin
    ...
    if (response_accept) begin
      ...
      assert (!axi_rlast_i || response_half_v)
        else $error("Prefetch AXI bridge received AXI RLAST on half a BedRock beat");
    end
  end
