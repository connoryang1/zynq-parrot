...
- **Midpoint 42 classified good (good 42, bad 57; 15 remain):** routed candidate `3992f34d` / `cbc0a57e` fit at 40,232 LUTs and 34 BRAM with WNS +2.007 ns and WHS +0.009 ns. Its hash-verified archived Linux run reached `/init`, completed rootfs checks and clean poweroff, and ended in `CORE[0] PASS`; phase advances to **constructing midpoint 49/113**.
...
- **FPGA-fit endpoint selected (110/113):** the repaired 113-commit endpoint passed Linux-entry, integer/FP nonresident rings, and the 5-warm/13-cold benchmark locally, but definitively failed placement at 62,948 LUTs because its final three nonresident-FP-copy commits exceed the xc7z020. Pushed GPR-fit endpoint `7fd64a63` / `b5fe1e93` retains the complete SRAM-backed integer path, Linux replay fix, normal FP execution, and 13-cycle result; its local Linux-entry and repeated nonresident ring pass, and routed job `20260905T141747Z-7fd64a63` is running.

- **FPGA-fit endpoint classified Linux-bad (fixed 55, bad 110; 55 remain):** exact endpoint `7fd64a63` / `b5fe1e93` routed successfully at 47,710 LUTs and 80 BRAM tiles with WNS +0.672 ns and WHS +0.002 ns, but its hash-verified archived-Linux run reproducibly stopped after the two atomic DMA-pool messages at kernel time 1.324745 s until the clean 180-second target limit. The original commit-55 bug remains fixed; this establishes a second regression somewhere in feature commits 56--110.
