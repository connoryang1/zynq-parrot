...
| Keep custom NBF, simulator, and bitstream dimensions identical | A 2-resident/4-context NBF cannot run against the stock configuration. | `3d2eb55`, `8c690a1`, `5c8ecd5`, `2654fac`; the exact-validation script and FPGA skill now enforce this. |
...
| 2 | Run a routed PYNQ-Z2 Vivado build with `CFG=e_bp_custom_cfg NUM_THREADS=2 NUM_CONTEXTS=4`; check fit, timing, and package integrity. |
...
- A future Linux-resident context-switch C demo comes **after** this baseline
  boot is restored; it must use the same custom context configuration.
